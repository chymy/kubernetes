...
func (m *GracefulTerminationManager) deleteRsFunc(rsToDelete *listItem) (bool, error) {
	klog.V(5).Infof("Trying to delete rs", "name", rsToDelete.String())
	...
	for _, rs := range rss {
		if rsToDelete.RealServer.Equal(rs) {
			...
			if utilipvs.IsRsGracefulTerminationNeeded(rsToDelete.VirtualServer.Protocol) && rs.ActiveConn+rs.InactiveConn != 0 {
				klog.V(5).Infof("Not deleting rs", "name", rsToDelete.String(), "ActiveConn", rs.ActiveConn, "InactiveConn", rs.InactiveConn)
				return false, nil
			}
			klog.V(5).Infof("Deleting rs", "name", rsToDelete.String())
